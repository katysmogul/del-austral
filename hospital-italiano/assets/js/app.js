...
function svgMascota(ancho) {
  return `<img class="mascota" src="assets/img/mascota.png" alt="Mascota" style="width:${ancho}px; height:${ancho}px; margin:0 auto 14px; border-radius:50%; object-fit:cover; display:block;">`;
}
...
async function llamarApi(url, opciones = {}) {
  try {
    ...
    if (!respuesta.ok || !datos.ok) {
      ...
      if (datos.mantenimiento) error.mantenimiento = true;
      throw error;
    }
    return datos;
  } catch (err) {
...
let MANTENIMIENTO_ACTIVO = false;
let SUSPENDIDA_POR_PAGO_ACTIVA = false;
...
let NOMBRE_INSTITUCION_ACTUAL = 'Del Austral';

/**
 * Reemplaza "Del Austral" por el nombre real de la institución
 * en todos los lugares visibles de la interfaz (título de la
 * pestaña, nombre PWA para iOS, encabezados de login y barra
 * superior). Si no llega ningún nombre (instalación vieja sin
 * migrar), no hace nada y queda el valor por defecto.
 */
function aplicarNombreInstitucion(nombre) {
  if (!nombre) return;
  NOMBRE_INSTITUCION_ACTUAL = nombre;
  const titulo = document.getElementById('title-pagina');
  if (titulo) titulo.textContent = `${nombre} · Historial Clínico`;
  const metaApple = document.getElementById('meta-apple-titulo');
  if (metaApple) metaApple.setAttribute('content', nombre);
  const tituloLogin = document.getElementById('titulo-marca-login');
  if (tituloLogin) tituloLogin.textContent = nombre;
  const tituloTopbar = document.getElementById('titulo-marca-topbar');
  if (tituloTopbar) tituloTopbar.textContent = nombre;
}

function mostrarPantallaMantenimiento() {
  MANTENIMIENTO_ACTIVO = true;
  document.getElementById('vista-acceso').classList.add('oculto');
  document.getElementById('vista-mantenimiento').classList.remove('oculto');
  // El botón de Apoderado sigue visible y funcional a propósito:
  // es el único rol que puede seguir entrando durante el mantenimiento.
}

function mostrarPantallaSuspendidaPorPago() {
  SUSPENDIDA_POR_PAGO_ACTIVA = true;
  document.getElementById('vista-acceso').classList.add('oculto');
  document.getElementById('vista-suspendida-pago').classList.remove('oculto');
  // El botón de Apoderado sigue visible y funcional a propósito:
  // es el único rol que puede seguir entrando para ver la deuda y pagar.
}
...
async function inicializarAcceso() {
  ...
  try {
    ...
    aplicarNombreInstitucion(res.nombre_institucion);

    // Nota: si el mantenimiento (o la suspensión por pago) se
    // activa ANTES de que exista la clave de Apoderado
    // (institución recién creada, sin terminar su configuración
    // inicial), esta pantalla asume que ya hay una clave para
    // ingresar. Es un caso borde muy poco probable en la
    // práctica.
    if (res.mantenimiento_activo) {
      mostrarPantallaMantenimiento();
      return;
    }
    if (res.suspendida_por_pago_activa) {
      mostrarPantallaSuspendidaPorPago();
      return;
    }
  } catch (e) {
  ...
  if (ETAPA_SISTEMA === 'falta_firma_contrato') {
    inicializarCanvasFirmaContratoCliente();
    mostrarPasoLogin('paso-firmar-contrato');
  } else if (ETAPA_SISTEMA === 'sin_desarrollador') {
    ...
    // link para entrar como Apoderado si hace falta configurar.
    cargarSedesLogin();
...
function vincularPasosLogin() {
  // --- Crear clave de apoderado (primera vez) ---
  ...
  // --- Acceso al rol Apoderado ---
  document.getElementById('btn-acceso-mantenimiento').addEventListener('click', () => {
    // Si el sistema está en mantenimiento o suspendido por pago,
    // esas pantallas están ocultas a propósito: el Apoderado es
    // el único que puede "salir" de ellas para llegar al
    // formulario de clave.
    document.getElementById('vista-mantenimiento').classList.add('oculto');
    document.getElementById('vista-suspendida-pago').classList.add('oculto');
  ...
  // --- Clave de apoderado (login normal) ---
  document.getElementById('btn-volver-sede-desde-dev').addEventListener('click', () => {
    // Si el sistema sigue en mantenimiento o suspendido por
    // pago, "volver" no debe llevar al login normal (nadie más
    // puede entrar de todos modos) sino a la pantalla de bloqueo
    // que corresponda.
  ...
  // --- Setup inicial (sede + profesional, justo después de crear apoderado) ---
  document.getElementById('btn-crear-setup-inicial').addEventListener('click', crearSetupInicial);
  document.getElementById('btn-ir-panel-dev').addEventListener('click', () => entrarAlApp('Apoderado', 'desarrollador'));
}
...
async function crearSetupInicial() {
  ...
  try {
    ...
    entrarAlApp('Apoderado', 'desarrollador');
  } catch (e) {
    mostrarToast(e.message, 'error');
    btn.disabled = false;
...
async function cargarSedesLogin() {
  ...
  try {
    ...
    cont.innerHTML = '';
    if (!res.datos.length) {
      cont.innerHTML = `<div class="texto-vacio-login">${svgMascota(48)}Todavía no hay sedes configuradas. Entrá como Apoderado para crear la primera.</div>`;
      return;
    }
    res.datos.forEach(s => {
    ...
    res.datos.forEach(p => {
      ...
      fila.innerHTML = `
        <div class="info-principal" style="cursor:pointer;">
          <div class="avatar-iniciales" style="background:#FCE4E9; color:#E63757;">${(p.nombre[0] + p.apellido[0]).toUpperCase()}</div>
          <div>
...
async function montarVistaConfiguracion(contenido) {
  ...
  // El Apoderado ya está en su única pantalla: no tiene
  ...
  const paneles = { sedes: 'panel-config-sedes', usuarios: 'panel-config-usuarios', historial: 'panel-config-historial', version: 'panel-config-version', contrato: 'panel-config-contrato', facturacion: 'panel-config-facturacion', soporte: 'panel-config-soporte', reportes: 'panel-config-reportes', papelera: 'panel-config-papelera', huerfanos: 'panel-config-huerfanos', calendario: 'panel-config-calendario', 'papelera-derivaciones': 'panel-config-papelera-derivaciones' };
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      tabs.forEach(t => t.classList.remove('activo'));
      tab.classList.add('activo');
      ...
      if (destino === 'facturacion') cargarEstadoCobroCliente();
      if (destino === 'soporte') cargarMisReportesBug();
  ...
  const inputNombreInst = document.getElementById('input-nombre-institucion');
  inputNombreInst.value = NOMBRE_INSTITUCION_ACTUAL;
  document.getElementById('btn-guardar-nombre-institucion').addEventListener('click', async () => {
    const nombreNuevo = inputNombreInst.value.trim();
    if (!nombreNuevo) { mostrarToast('Ingresá un nombre para la institución.', 'error'); return; }
    const btn = document.getElementById('btn-guardar-nombre-institucion');
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner"></span> Guardando...';
    try {
      await llamarApi(API.auth, { method: 'POST', body: JSON.stringify({ accion: 'cambiar_nombre_institucion', nombre: nombreNuevo }) });
      mostrarToast('Nombre actualizado correctamente.', 'exito');
      aplicarNombreInstitucion(nombreNuevo);
    } catch (e) {
      mostrarToast(e.message, 'error');
    } finally {
      btn.disabled = false;
...
function crearFilaPapeleraDev(reg) {
  ...
  fila.innerHTML = `
    <div class="info-principal">
      <div class="avatar-iniciales" style="background:#FCE4E9; color:#E63757;">
...
let SELECT_SEDE_BUG_POBLADO = false;

async function poblarSelectSedeBug() {
  if (SELECT_SEDE_BUG_POBLADO) return;
  const select = document.getElementById('select-sede-bug');
  try {
    const res = await llamarApi(API.auth, { method: 'POST', body: JSON.stringify({ accion: 'listar_sedes' }) });
    (res.datos || []).forEach(sede => {
      const opt = document.createElement('option');
      opt.value = sede.nombre;
      opt.textContent = sede.nombre;
      select.appendChild(opt);
    });
    SELECT_SEDE_BUG_POBLADO = true;
  } catch (e) {
    // Si falla, el selector queda con la opción "No aplica" nomás.
  }
}

function badgeEstadoBug(estado) {
  const mapa = {
    nuevo: { texto: 'Nuevo', clase: 'nuevo' },
    visto: { texto: 'Visto por soporte', clase: 'visto' },
    en_curso: { texto: 'En curso', clase: 'en-curso' },
    resuelto: { texto: 'Resuelto', clase: 'resuelto' },
    no_resuelto: { texto: 'No resuelto', clase: 'no-resuelto' },
  };
  return mapa[estado] || { texto: estado, clase: '' };
}

function etiquetaSeveridadBug(severidad) {
  const mapa = {
    visual: '🎨 Bug visual',
    funcional: '⚙️ No funciona',
    critico: '🚨 Crítico',
...
// ============================================================
// FACTURACIÓN / ESTADO DE COBRO (panel Apoderado)
// ============================================================

function badgeEstadoCobro(estado) {
  const mapa = {
    pendiente: { texto: 'Pendiente de pago', clase: 'nuevo' },
    comprobante_subido: { texto: 'Comprobante en revisión', clase: 'en-curso' },
    aprobado: { texto: 'Pago aprobado', clase: 'resuelto' },
    sin_acreditar: { texto: 'Sin acreditar', clase: 'nuevo' },
    rechazado: { texto: 'Rechazado', clase: 'no-resuelto' },
  };
  return mapa[estado] || { texto: estado, clase: '' };
}

async function cargarEstadoCobroCliente() {
  const cont = document.getElementById('contenido-estado-cobro');
  const badgeTab = document.getElementById('badge-pago-pendiente');
  cont.innerHTML = '<div class="cargando-pagina chico"><span class="spinner"></span></div>';

  try {
    const res = await llamarApi(`${API.cobranza}?accion=obtener_estado_cobro`, { method: 'POST', body: JSON.stringify({ accion: 'obtener_estado_cobro' }) });
    const datos = res.datos;

    if (!datos || !datos.cobro) {
      cont.innerHTML = `<div class="resumen-vacio">Todavía no hay ningún cobro generado para esta institución.${datos && datos.saldo_favor > 0 ? `<br><strong>Saldo a favor: $${datos.saldo_favor.toLocaleString('es-AR')}</strong>` : ''}</div>`;
      badgeTab.classList.add('oculto');
      return;
    }

    const cobro = datos.cobro;
    const estadoInfo = badgeEstadoCobro(cobro.estado);
    const hoy = new Date();
    const vencimiento = new Date(cobro.vencimiento + 'T00:00:00');
    const vencido = vencimiento < hoy && !['aprobado'].includes(cobro.estado);
    const fechaFormateada = vencimiento.toLocaleDateString('es-AR');

    badgeTab.classList.toggle('oculto', !['pendiente', 'sin_acreditar', 'rechazado'].includes(cobro.estado));

    const puedeSubirComprobante = ['pendiente', 'sin_acreditar', 'rechazado'].includes(cobro.estado);
    const montoMostrado = vencido ? cobro.monto_con_recargo : cobro.monto;

    cont.innerHTML = `
      <div class="fila-reporte-bug" style="margin-bottom:20px;">
        <div class="cabecera-reporte-bug">
          <strong>$${montoMostrado.toLocaleString('es-AR', { minimumFractionDigits: 2 })} ${cobro.moneda}</strong>
          <span class="badge-estado-bug ${estadoInfo.clase}">${estadoInfo.texto}</span>
        </div>
        <div class="meta-reporte-bug">
          Vencimiento: ${fechaFormateada}${vencido ? ' <strong style="color:var(--coral);">(vencido)</strong>' : ''}
        </div>
        ${cobro.descuento_pct > 0 ? `<div class="meta-reporte-bug" style="margin-top:4px; color:var(--salvia-oscuro);">Incluye un descuento comercial del ${cobro.descuento_pct}% sobre el valor de lista ($${cobro.monto_lista.toLocaleString('es-AR')}).</div>` : ''}
        ${cobro.saldo_favor_aplicado > 0 ? `<div class="meta-reporte-bug" style="margin-top:4px; color:var(--salvia-oscuro);">Se aplicó un saldo a favor de $${cobro.saldo_favor_aplicado.toLocaleString('es-AR')} a este cobro.</div>` : ''}
        ${vencido && cobro.recargo_mora_vigente > 0 ? `<div class="meta-reporte-bug" style="margin-top:4px; color:var(--coral);">Incluye un recargo por mora de $${cobro.recargo_mora_vigente.toLocaleString('es-AR', { minimumFractionDigits: 2 })} (2,5% diario + IVA desde el vencimiento). Este recargo sigue creciendo cada día hasta que se registre el pago.</div>` : ''}
        ${cobro.notas_super_admin ? `<div class="respuesta-soporte-bug"><strong>Nota del Prestador:</strong> ${escaparHtml(cobro.notas_super_admin)}</div>` : ''}
      </div>

      ${!vencido && cobro.estado === 'pendiente' ? `<p class="ayuda" style="margin-bottom:16px;">Si el pago no se registra antes del vencimiento, se aplica un recargo del 2,5% diario más IVA sobre el monto, hasta que se acredite.</p>` : ''}

      ${datos.saldo_favor > 0 ? `<p class="ayuda" style="margin-bottom:16px;">Tenés un saldo a favor de <strong>$${datos.saldo_favor.toLocaleString('es-AR')}</strong>, que se aplica automáticamente al próximo cobro que se genere.</p>` : ''}

      <div class="panel-plantilla-nueva">
        <p class="panel-subtitulo" style="margin-bottom:12px;">Datos para la transferencia</p>
        <div class="grilla-datos" style="margin-bottom:20px;">
          <div class="dato-box"><div class="etiqueta-dato">Titular</div><div class="valor-dato">${escaparHtml(datos.cuenta.titular)}</div></div>
          <div class="dato-box"><div class="etiqueta-dato">Banco</div><div class="valor-dato">${escaparHtml(datos.cuenta.banco)}</div></div>
          <div class="dato-box"><div class="etiqueta-dato">CUIL/CUIT</div><div class="valor-dato">${escaparHtml(datos.cuenta.cuil)}</div></div>
          <div class="dato-box" style="grid-column: 1 / -1;"><div class="etiqueta-dato">Número de cuenta / CBU</div><div class="valor-dato">${escaparHtml(datos.cuenta.numero)}</div></div>
          ${datos.cuenta.alias ? `<div class="dato-box"><div class="etiqueta-dato">Alias</div><div class="valor-dato">${escaparHtml(datos.cuenta.alias)}</div></div>` : ''}
        </div>

        ${puedeSubirComprobante ? `
          <p class="panel-subtitulo" style="margin-bottom:10px;">Subir comprobante de pago</p>
          <input type="file" id="input-archivo-comprobante" accept="application/pdf,image/jpeg,image/png" style="margin-bottom:10px;">
          <button class="btn btn-primario" id="btn-subir-comprobante" data-cobro-id="${cobro.id}">Subir comprobante</button>
          <p id="error-subir-comprobante" class="instruccion-patron error oculto" style="margin-top:10px;"></p>
        ` : cobro.estado === 'comprobante_subido' ? `
          <p class="ayuda">Ya subiste el comprobante. El Prestador lo va a revisar y te va a avisar cuando quede aprobado.</p>
        ` : ''}
      </div>
    `;

    if (puedeSubirComprobante) {
      document.getElementById('btn-subir-comprobante').addEventListener('click', async () => {
        const errorP = document.getElementById('error-subir-comprobante');
        errorP.classList.add('oculto');
        const inputArchivo = document.getElementById('input-archivo-comprobante');
        if (!inputArchivo.files.length) {
          errorP.textContent = 'Elegí un archivo antes de subir.';
          errorP.classList.remove('oculto');
          return;
        }

        const archivoElegido = inputArchivo.files[0];
        // Aviso preventivo: muchos servidores rechazan archivos
        // grandes (413 Request Entity Too Large) con un límite
        // más chico que el que permite este sistema. Si la foto
        // pesa mucho (común en fotos de celular sin comprimir),
        // avisamos antes de intentar subirla.
        if (archivoElegido.size > 8 * 1024 * 1024) {
          errorP.innerHTML = `El archivo pesa ${(archivoElegido.size / (1024*1024)).toFixed(1)} MB. Si el servidor lo rechaza, probá con una foto más liviana (podés sacarle una captura de pantalla en baja calidad, o comprimirla antes de subirla).`;
          errorP.classList.remove('oculto');
        }
        ...
        const formData = new FormData();
        formData.append('cobro_id', btn.dataset.cobroId);
        formData.append('comprobante', archivoElegido);

        try {
          const respuesta = await fetch(`${API.cobranza}?accion=subir_comprobante`, { method: 'POST', body: formData });

          let resultado;
          try {
            resultado = await respuesta.json();
          } catch (errorParseo) {
            // La respuesta no es JSON: normalmente significa que
            // el propio servidor web rechazó la subida antes de
            // que PHP llegara a procesarla (por ejemplo, 413
            // Request Entity Too Large por el tamaño del
            // archivo, o 502/504 si el servidor tardó demasiado).
            if (respuesta.status === 413) {
              throw new Error('El servidor rechazó el archivo porque es demasiado grande. Probá con una imagen más liviana o comprimida.');
            }
            throw new Error(`El servidor no pudo procesar la subida (código ${respuesta.status}). Probá con un archivo más chico, o avisá a soporte si persiste.`);
          }

          if (!respuesta.ok || !resultado.ok) throw new Error(resultado.error || 'No se pudo subir el comprobante.');
          mostrarToast('Comprobante enviado. Vamos a revisarlo pronto.', 'exito');
          cargarEstadoCobroCliente();
        } catch (e) {
    ...
    cont.innerHTML = `<p class="resumen-vacio">No se pudo cargar el estado de cobro: ${escaparHtml(e.message)}</p>`;
  }
}

async function cargarMisReportesBug() {
  poblarSelectSedeBug();

  const btn = document.getElementById('btn-enviar-reporte-bug');
  const errorP = document.getElementById('error-reporte-bug');
  if (!btn.dataset.vinculado) {
    btn.dataset.vinculado = '1';
    btn.addEventListener('click', async () => {
      errorP.classList.add('oculto');
      const titulo = document.getElementById('input-titulo-bug').value.trim();
      const descripcion = document.getElementById('input-descripcion-bug').value.trim();
      const severidad = document.getElementById('select-severidad-bug').value;
      const sedeNombre = document.getElementById('select-sede-bug').value;

      if (!titulo || !descripcion) {
        errorP.textContent = 'Completá el título y la descripción antes de enviar.';
        errorP.classList.remove('oculto');
        return;
      }
      ...
      try {
        await llamarApi(API.auth, {
          method: 'POST',
          body: JSON.stringify({ accion: 'crear_reporte_bug', titulo, descripcion, severidad, sede_nombre: sedeNombre }),
        });
        document.getElementById('input-titulo-bug').value = '';
        document.getElementById('input-descripcion-bug').value = '';
        mostrarToast('Reporte enviado. Gracias por avisar.', 'exito');
        cargarListaMisReportesBug();
      } catch (e) {
        errorP.textContent = e.message;
        errorP.classList.remove('oculto');
      } finally {
...
async function cargarListaMisReportesBug() {
  const cont = document.getElementById('lista-mis-reportes-bug');
  cont.innerHTML = '<div class="cargando-pagina chico"><span class="spinner"></span></div>';
  try {
    const res = await llamarApi(`${API.auth}?accion=listar_mis_reportes_bug`, { method: 'POST', body: JSON.stringify({ accion: 'listar_mis_reportes_bug' }) });
    if (!res.datos.length) {
      cont.innerHTML = '<p class="resumen-vacio">Todavía no reportaste ningún problema.</p>';
      return;
    }
    cont.innerHTML = '';
    res.datos.forEach(r => {
      const estado = badgeEstadoBug(r.estado);
      const fecha = new Date(r.creado_en).toLocaleDateString('es-AR');
      const fila = document.createElement('div');
      fila.className = 'fila-reporte-bug';
      fila.innerHTML = `
        <div class="cabecera-reporte-bug">
          <strong>${escaparHtml(r.titulo)}</strong>
          <span class="badge-estado-bug ${estado.clase}">${estado.texto}</span>
        </div>
        <div class="meta-reporte-bug">${etiquetaSeveridadBug(r.severidad)} · ${r.sede_nombre ? escaparHtml(r.sede_nombre) + ' · ' : ''}${fecha}</div>
        <p class="descripcion-reporte-bug">${escaparHtml(r.descripcion)}</p>
        ${r.respuesta_soporte ? `<div class="respuesta-soporte-bug"><strong>Respuesta de soporte:</strong> ${escaparHtml(r.respuesta_soporte)}</div>` : ''}
      `;
      cont.appendChild(fila);
    });
  } catch (e) {
    cont.innerHTML = `<p class="resumen-vacio">No se pudo cargar tus reportes: ${escaparHtml(e.message)}</p>`;
  }
}
    ...
    if (filtro === 'derivacion') {
      ...
      res.datos.forEach(d => {
        ...
        btnEliminar.addEventListener('click', async () => {
          if (!confirm(`¿Eliminar el resumen de ${d.nombre_completo}? Queda guardado en la papelera, el Apoderado lo puede recuperar.`)) return;
          try {
        ...
        btnEliminarDefinitivo.addEventListener('click', async () => {
          if (!confirm(`¿Eliminar PARA SIEMPRE el resumen de ${d.nombre_completo}? Esta acción no se puede deshacer, ni siquiera el Apoderado va a poder recuperarlo.`)) return;
          try {
...
function inicializarCanvasFirmaContratoCliente() {
  const linkContrato = document.getElementById('link-ver-contrato-antes-firmar');
  if (linkContrato) linkContrato.href = 'ver_contrato.php';

  const canvas = document.getElementById('canvas-firma-contrato-cliente');
  ...
  document.getElementById('btn-limpiar-firma-contrato-cliente').addEventListener('click', () => {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    huboTrazo = false;
  });

  const errorP = document.getElementById('error-firma-contrato-cliente');
  document.getElementById('btn-confirmar-firma-contrato-cliente').addEventListener('click', async () => {
    errorP.classList.add('oculto');
    if (!huboTrazo) {
      errorP.textContent = 'Dibujá tu firma antes de continuar.';
      errorP.classList.remove('oculto');
      return;
    }
    const btn = document.getElementById('btn-confirmar-firma-contrato-cliente');
    btn.disabled = true;
    try {
      await llamarApi(API.auth, {
        method: 'POST',
        body: JSON.stringify({ accion: 'firmar_contrato_cliente', firma_png: canvas.toDataURL('image/png') }),
      });
      ETAPA_SISTEMA = 'sin_desarrollador';
      mostrarPasoLogin('paso-crear-desarrollador');
    } catch (e) {
      errorP.textContent = e.message;
      errorP.classList.remove('oculto');
...
async function cargarHistorialCambios(pagina) {
  ...
  // Apoderado (es el único que ve TODO el historial mezclado).
  if (selectFiltro) {
