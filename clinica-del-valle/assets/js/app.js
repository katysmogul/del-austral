...
function vincularPasosLogin() {
  ...
  // --- Acceso discreto al rol Apoderado ---
...
async function cargarSedesLogin() {
  ...
  try {
    ...
    cont.innerHTML = '';
    if (!res.datos.length) {
      cont.innerHTML = '<p class="texto-vacio-login">Todavía no hay sedes configuradas. Entrá como Apoderado para crear la primera.</p>';
      return;
    }
    res.datos.forEach(s => {
