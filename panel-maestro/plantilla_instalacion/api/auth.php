...
if ($accion === 'estado') {
    ...
    if (!$hayDesarrollador) {
        ...
        if (!$hayFirma) {
            echo json_encode(['ok' => true, 'etapa' => 'falta_firma_contrato', 'nombre_institucion' => $nombreInst]);
            exit;
        }
        echo json_encode(['ok' => true, 'etapa' => 'sin_desarrollador', 'largo_pin' => LARGO_PIN, 'nombre_institucion' => $nombreInst]);
        exit;
    }
    ...
    if ($sedesConUsuarios == 0) {
        echo json_encode(['ok' => true, 'etapa' => 'sin_sedes_o_usuarios', 'largo_pin' => LARGO_PIN, 'nombre_institucion' => $nombreInst]);
        exit;
    }

    echo json_encode(['ok' => true, 'etapa' => 'listo', 'largo_pin' => LARGO_PIN, 'nombre_institucion' => $nombreInst]);
    exit;
}
