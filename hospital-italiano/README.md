...
- 🛠️ **Rol Apoderado**: nivel de acceso separado y discreto (sin mención en la pantalla de login pública), dedicado a crear/renombrar sedes y gestionar profesionales y administrativas. Antes de poder crear esta clave por primera vez, hay que firmar el contrato de servicio (ver más abajo).
...
- 📅 **Calendario de vencimientos de licencia** y aviso anticipado de los que vencen en la próxima semana, para el Apoderado.
...
- 🧾 **Historial de cambios (auditoría)**: acotado a los propios pacientes de cada profesional; el Apoderado ve el historial completo de todos, con filtro por tipo de entidad.
...
- 🎫 **Servicios Plus**: cuatro documentos exportables a PDF con legajo existente o carga manual sin necesidad de legajo — **Constancia de asistencia**, **Constancia de tratamiento prolongado** y **Receta** (con sello/firma automática y token público de validación que vence a los 90 días), y **Resumen de derivación** (sin token, para compartir directamente con otro profesional o institución, sin vencimiento automático, pero con su propia papelera: eliminar/recuperar o eliminar para siempre).
- 🏷️ **Nombre de institución personalizable**: "Del Austral" no está fijo en el código — se configura desde el panel del Apoderado y se aplica en toda la interfaz (título, login, PDF exportados, app instalable), pensado para poder ofrecer el sistema a distintas instituciones con su propia identidad.
- 📜 **Contrato de servicio firmado digitalmente**: antes de crear la clave de Apoderado por primera vez, hay que leer y firmar (con canvas) el contrato de servicio de la institución. Queda disponible para verlo en cualquier momento desde Configuración → "Contrato".
- 🐞 **Reportes de bug**: el Apoderado puede avisar errores o sugerencias con categoría/severidad y sede opcional, y ver el estado de cada uno (nuevo, visto, en curso, resuelto, no resuelto) junto con la respuesta del equipo de soporte.
- 🩺 **Salud del sistema (panel maestro)**: el Super Admin ve, por institución y sin conectarse a mano, si los cron corrieron, si hay actividad reciente, si la versión está al día, y si hubo errores de la app en las últimas 24 horas — con auto-refresco cada minuto.
- 🚧 **Modo mantenimiento global (panel maestro)**: un botón bloquea el ingreso a todas las instituciones a la vez, con una pantalla clara para quien intente entrar. El Apoderado de cada institución sigue pudiendo acceder mientras está activo.
- 💳 **Sistema de cobranza (panel maestro + Apoderado)**: el Super Admin genera cobros manuales con monto, descuento comercial (5/10/15/20/30%) y vencimiento; el Apoderado ve el estado, los datos bancarios, y sube el comprobante (PDF/JPG/PNG) desde su propio panel. El Super Admin aprueba, marca sin acreditar, rechaza o anula (por dato erróneo, sin borrar el historial), gestiona saldo a favor (se descuenta solo del próximo cobro), y puede suspender el acceso específicamente por falta de pago (bloquea login normal, pero el Apoderado sigue entrando a pagar, y se reactiva sola al aprobarse el cobro). Cada cobro vencido sin pagar acumula un recargo automático del 2,5% diario más IVA. Cada cobro genera una factura navegable por mes, con marca de agua PAGADA/NO PAGADA/ANULADA.
...
├── migracion_v16.sql            # Papelera de resúmenes de derivación
├── migracion_v17.sql            # Nombre de institución personalizable (white-label)
├── manifest.php                  # Manifest dinámico de la PWA (reemplaza manifest.json), con el nombre real de la institución
├── exportar_constancia.php       # Vista de impresión/exportación de constancia/tratamiento/receta a PDF
├── exportar_derivacion.php        # Vista de impresión/exportación de resumen de derivación a PDF (sin token)
├── validar_constancia.php         # Página pública (sin login) para validar constancias por token
├── ver_contrato.php                # Muestra el contrato.html sincronizado desde el panel maestro, sin login (se usa antes de poder crear la clave de Apoderado)
├── cron_limpiar_constancias.php    # Script que borra constancias vencidas (90 días), correr vía cron
├── cron_limpiar_reportes_bug.php    # Script que borra reportes de bug resueltos/no resueltos con más de 7 días, correr vía cron
├── config/
│   └── config.php               # Credenciales de BD + helpers de sesión/rol/auditoría
├── api/
│   ├── auth.php                  # Login multi-paso, Apoderado, sedes, usuarios, legajos, firma de contrato, reportes de bug
...
4. Abrí el sitio: la primera vez te va a pedir leer y firmar el contrato de servicio (canvas), y recién después te deja crear la clave de Apoderado, y luego tu primera sede y profesional.

Si venís de una versión anterior con pacientes ya cargados, corré las migraciones en orden (`migracion_v2.sql` hasta `migracion_v17.sql`), sin saltarte ninguna. Ningún script borra pacientes, sesiones, citas ni adjuntos.
...
- El **Apoderado** entra por un botón visible en la esquina superior derecha que dice "Apoderado", con su propia clave separada. Antes de poder crear esa clave por primera vez, tiene que leer y firmar el contrato de servicio con un canvas — sin esa firma, el botón de crear la clave queda bloqueado. No ve pacientes; gestiona sedes, profesionales, administrativas y licencias, y también puede reportar bugs o sugerencias al equipo de soporte desde Configuración.
...
- `desarrollador` — clave única del rol Apoderado (el nombre de la tabla quedó igual por compatibilidad; el rol se muestra como "Apoderado" en toda la interfaz)
- `contrato_firma_cliente` — firma del Apoderado sobre el contrato de servicio, requerida antes de poder crear la clave por primera vez
- `reportes_bug` — reportes de errores o sugerencias enviados por el Apoderado, con estado y respuesta del equipo de soporte
- `salud_sistema` — clave/valor genérico: última corrida de cada cron, última actividad real. Lo lee el panel maestro para el panel de Salud del Sistema
- `errores_app` — log de errores fatales/excepciones de PHP, capturado por la propia app (no depende del log del hosting)
...
- Los PIN y la clave de Apoderado se almacenan con `password_hash()` (bcrypt) combinados con un secreto de aplicación (`APP_SECRET`); nunca en texto plano.
...
- El acceso de Apoderado es visible en la pantalla de login (botón "Apoderado" arriba a la derecha), protegido por su propia clave.
- Se recomienda servir el sitio bajo HTTPS dado que se transmiten datos de salud (ver el detalle de certificado SSL en `INSTRUCCIONES_INSTALACION.md`).
...
Hay dos scripts pensados para correr una vez al día vía cron: `cron_limpiar_constancias.php` (borra constancias vencidas a los 90 días) y `cron_limpiar_reportes_bug.php` (borra reportes de bug resueltos o no resueltos con más de 7 días desde ese cambio de estado). Ninguno de los dos es obligatorio para que el sistema funcione — si no se programan, simplemente esos registros se van acumulando sin borrarse solos. La guía completa de cómo programarlos (con ejemplos para VPS y cPanel, y qué hacer si tenés varias instituciones instaladas) está en **[`INSTRUCCIONES_INSTALACION.md`](./INSTRUCCIONES_INSTALACION.md#tareas-programadas-cron-y-certificado-ssl--resumen-final)**.

## Hoja de ruta

Ideas pendientes, sin compromiso de implementación:

- [ ] Recuperación de PIN olvidado sin pasar por phpMyAdmin (hoy existe el cambio de PIN por el Apoderado, pero no un flujo de autorrecuperación)
